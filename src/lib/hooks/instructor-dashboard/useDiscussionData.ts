...
export function useDiscussionData({
  ...
  const filterMessagesByTime = useCallback((filterTimePercent: number) => {
    ...
    const discussionDuration = discussionDurationRef.current;
    
    if (!messages || !discussionStartTime || discussionDuration === 0) {
      console.log("Cannot filter messages - missing data:", {
        hasMessages: !!messages,
        messageCount: messages?.length || 0,
        hasStartTime: !!discussionStartTime,
        hasDuration: discussionDuration > 0,
        duration: discussionDuration
      });
      return messages || []; // Return all messages instead of empty array if data is missing
    }

    // If filter is at 0%, start with showing at least some messages (first 10%)
    if (filterTimePercent <= 10) {
      // Show at least the first 10% of messages based on their timestamps
      const sortedMessages = [...messages].sort((a, b) => {
        return new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
      });
      const initialCount = Math.max(1, Math.floor(sortedMessages.length * 0.1));
      return sortedMessages.slice(0, initialCount);
    }
    ...
    const filterTime = discussionStartTime.getTime() + (discussionDuration * filterTimePercent / 100);
    
    const filtered = messages.filter(msg => {
      // Safely parse the timestamp with a more robust approach
      let msgTime;
      try {
        // Handle the PostgreSQL timestamp format explicitly
    ...
    // Always return at least some messages
